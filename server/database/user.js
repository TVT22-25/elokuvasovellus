...
const deleteUser = async (user_id, group_id) => {
  const result = await pgPool.query(sql.DELETE_USER, [user_id, group_id]);
  return result.rowCount > 0
    ? { code: 202, content: result.rows }
    : { code: 404, content: { error: 'user not found by user_id' } };
};

module.exports = { getUsers, getUserGroups, postUser, loginUser, updateUser, deleteUser };
